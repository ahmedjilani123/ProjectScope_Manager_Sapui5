sap.ui.define([
    ...
    return Controller.extend("cal.as.sap.calendarcustom.controller.Basecontroller", {
        SelectionYearChange(year = null) {
            var currentYear = year != null ? new Date(year, 0, 1) : new Date();
                ...
                se.item.forEach((itemUser) => {
                    ...
                    GetMonthIndex.forEach((item, i) => {
                        ...
                        while (day <= item.lastDay) {
                            ...
                            dayObj.UserRole = itemUser.Value;
                            count++;
                            day += 1;
                        }
                    })
                ...
                while (day <= item.lastDay) {
                    columss = new sap.ui.table.Column({
                        ...
                        template: new sap.m.Button({
                            fieldGroupIds: '{TableCalendarSelect>FT' + item.year + day + item.month + '}' + '|' + '{TableCalendarSelect>fullDay' + item.year + day + item.month + '}',
                            type: {
                                parts: [
